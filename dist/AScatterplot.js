...
class AScatterplot extends EventEmitter {
    ...
    setTransform(transform) {
        ...
        const $zoom = select(this.parent).select(`.${cssprefix}-draw-area`);
        this.zoomBehavior
            .on('start', null)
            .on('zoom', null)
            .on('end', null);
    ...
    window2transform(window) {
        const range2transform = (minMax, scale) => {
    ...
    traverseTree(ctx, tree, renderer, xscale, yscale, isNodeVisible, debug = false, x, y) {
        //debug stats
